...
function getSpendingByCategory(transactions: Transaction[]) {
  ...
  for (const transaction of transactions) {
    if (transaction.transaction_type !== "OUTGOING" || !transaction.category) {
      continue;
    }
...
export function CategoryChart({ transactions }: SpendingByCategoryChartProps) {
  ...
  return (
    <View>
      <Text className={"mb-2 text-lg font-bold text-black dark:text-white"}>
        {i18n.t("chart.spendingByCategory")}
      </Text>
      <View
        className={
          "items-center rounded-2xl bg-surface dark:bg-surface-dark p-4"
        }
      >
